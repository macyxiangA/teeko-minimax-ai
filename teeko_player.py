...
class TeekoPlayer:
            ...
            for r in range(5):
                for c in range(5):
                    if state[r][c] == ' ':
                        ...
                        val = self.max_value(new_state, 1)
    ...
    def game_value(self, state):
        ...
        for i in range(5):
            for j in range(2):
                line = [state[i][j+k] for k in range(4)]
                if line.count(self.my_piece) == 4:
                    return 1
                if line.count(self.opp) == 4:
                    return -1
                line = [state[j+k][i] for k in range(4)]
                if line.count(self.my_piece) == 4:
                    return 1
                if line.count(self.opp) == 4:
                    return -1
        ...
        for r in range(2):
            for c in range(2):
                diag = [state[r+k][c+k] for k in range(4)]
                if diag.count(self.my_piece) == 4:
                    return 1
                if diag.count(self.opp) == 4:
                    return -1
        ...
        for r in range(4):
            for c in range(4):
                box = [state[r][c], state[r][c+1],
                       state[r+1][c], state[r+1][c+1]]
                if box.count(self.my_piece) == 4:
                    return 1
                if box.count(self.opp) == 4:
                    return -1
    ...
    def max_value(self, state, depth):
        """
        Helper function is to implement min-max as described in the writeup
        """
        ...
        if depth % 2 == 0:
            best_val = -1.0
            for succ_state in self.succ(state, self.my_piece):
                val = self.max_value(succ_state, depth + 1)
                if val > best_val:
                    best_val = val
            return best_val
        else:
            best_val = 1.0
            for succ_state in self.succ(state, self.opp):
                val = self.max_value(succ_state, depth + 1)
                if val < best_val:
                    best_val = val
            return best_val
